...
import { ZNorm as Normalization } from "./normalization.js";
...
class Graph {
    constructor(graph, options) {
        ...
        this._zn = new Normalization(this.edgesNum());
    ...
    testMove(nodeId, vec, effectBounds = false) {
        let node = this._nodes[nodeId];
        // need to reset the history since it effects future calculations and test move shouldn't effect anything
        let oldNormObject = this._zn.serialize();
        ...
        let oldPosMetricsNorm = this.normalizeAll(oldPosMetrics);
        this.normalizeAll(oldPosMetrics);

        node.x = x;
        node.y = y;

        let newPosMetrics = this.calcNodeMetrics(nodeId);
        let newPosMetricsNorm = this.normalizeAll(newPosMetrics);
        this.normalizeAll(newPosMetrics);

        this._normalMetrics = updateMetrics(
            this._normalMetrics,
            oldPosMetricsNorm,
            newPosMetricsNorm
        );
        let newObjective = this.objective();

        // restore graph state
        node.x = oldX;
        node.y = oldY;
        this._normalMetrics = {...oldNormalMetrics};
        this._zn.deserialize(oldNormObject);
    ...
    updateMetrics(oldMetrics, newMetrics) {
        updateMetrics(this._metrics, oldMetrics, newMetrics);
    }
    updateNormalMetrics(oldMetrics, newMetrics) {
        updateMetrics(this._normalMetrics, oldMetrics, newMetrics);
    }
}
Graph.status = {
...
function updateMetrics(currentMetrics, oldMetrics, newMetrics) {
    ...
    // what state to set? it could be that we updated the normal but not the metrics
    return metrics;
}
