...
export function hillClimbing_Fast_NoGrid(graph) {
    ...
    cost = ComputeCost(normalizedMetrics); // compute initial cost from solution vector

    while (square_size >= 1) {
        // drawing process
        Drawer3(square_size); // draw according to all measures
        EqualizeScales(graph.calcMetrics());
        cost = ComputeCost(EqualizeScales(graph.calcMetrics()));
        hcIterations++;
            ...
            for (let offset of offsets) {
                ...
                if (withinBounds(bounds, points[i])) {
                    ...
                    if (!equal(newFit, oldFit) && newFit < oldFit) {
                        ...
                        cost = ComputeCost(normalizedMetrics);
