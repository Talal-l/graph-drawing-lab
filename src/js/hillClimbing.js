...
export class HillClimbing {
  ...
  step() {
    ...
    this.cost = this._objective(
      this.zn.equalizeScales(calcMetrics(this.graph, this.params))
    );
    if (
      (!equal(this.cost, this.old_cost) && this.cost >= this.old_cost) ||
      equal(this.cost, this.old_cost)
    ) {
      this.squareSize /= 4;
    }
