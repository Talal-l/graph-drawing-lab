...
export function calcMetrics(graph, params) {
  ...
  for (let i = 0; i < graph._nodes.length; i++) {
    if (weights.nodeOcclusion) {
      metrics.nodeOcclusion += nodeOcclusionN(
        graph,
        i,
        params.occlusionThreshold ** 2
      );
    }
    ...
    if (weights.edgeLength) {
      metrics.edgeLength += edgeLengthN(graph, params.requiredEdgeLength, i);
    }
    if (weights.edgeCrossing) {
      metrics.edgeCrossing += edgeCrossingN(graph, i);
    }
    if (weights.angularResolution) {
      metrics.angularResolution += angularResolution2N(
        graph,
        i,
...
export function calcNodeMetrics(graph, nodeId, params) {
  ...
  if (weights.edgeLength) {
    metrics.edgeLength = edgeLengthN(graph, params.requiredEdgeLength, nodeId);
  }
    ...
    for (let nIndex of adj[i]) {
      ...
      for (let j = 0; j < nodes.length; j++) {
        ...
        for (let nIndex2 of adj[j]) {
          // create line for edge
          if (i !== j && j !== nIndex && i !== nIndex2 && nIndex !== nIndex2) {
    ...
    for (let j = 0; j < adj[adj[p][i]].length; j++) {
      if (adj[adj[p][i]][j] !== p) {
        ...
        if (degree < angle_threshold) {
          // check only the angles which are less than the given threshold
          radian = (Math.PI / 180) * degree; // convert the degree to radian
          c += Math.abs((2 * Math.PI) / adj[adj[p][i]].length - radian); // add the following to the total cost: (((2*PI)/degree of the point i adjacent to p) - degree in radian between line1 and line2)
        }
      }
    }
  }
...
export function angularResolution2N(graph, p, threshold) {
  ...
  for (
    let i = 0;
    i < adjacency[p].length;
    i++ // for the first line (p,i)
  ) {
    // the following checks the angles whose common node is p
    for (
      let j = i + 1;
      j < adjacency[p].length;
      j++ // for the second line sharing the same point p as for the first line (p,j)
    ) {
    ...
    for (let j = 0; j < adjacency[adjacency[p][i]].length; j++) {
      if (adjacency[adjacency[p][i]][j] != p) {
        ...
        x2 =
          points[adjacency[adjacency[p][i]][j]].x - points[adjacency[p][i]].x; // x coordinate difference in line 2
        y2 =
          points[adjacency[adjacency[p][i]][j]].y - points[adjacency[p][i]].y; // y coordinate difference in line 2
        ...
        if (degree < threshold) {
          // check only the angles which are less than the given threshold
          c += Math.abs(
            (2 * Math.PI) / adjacency[adjacency[p][i]].length - radian
      ...
      for (let m = 0; m < nodes.length; m++) {
        if (m !== i && m !== adj[i][j]) {
          ...
          if (a1Rad <= Math.PI / 2 && a2Rad <= Math.PI / 2) {
            distSq = util.ptLineDistSq(p1.x, p1.y, p2.x, p2.y, p3.x, p3.y);
            if (distSq === 0) distSq = 1; // avoid division by zero
            sum += 1.0 / distSq;
          }
        }
    ...
    for (let k = 0; k < 360; k++) {
      if (angles[k]) {
        if (e1 === null) {
          ...
          if (e1.overlap) log.anglesDiff.push({ e1, e2: angles[k], deg: k });
        } else {
