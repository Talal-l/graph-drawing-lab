...
export async function BatchRunPage() {
  ...
  function renderTabs() {
    ...
    for (let tab of tabs) {
      ...
      tabListEl.insertAdjacentHTML("beforeend", createTabEl(tab.title, tab.id));
      if (tab.runCount > 0) showTabSpinner(tab);
      else hideTabSpinner(tab);
    }
  ...
  function addTabContentEl(tab) {
    ...
    for (const [filename, file] of Object.entries(tab.files)) {
      ...
      let row = {
        ...
        edgeLength: {
          value: metrics ? metrics.edgeLength.toFixed(DIGITS) : "-",
          type: "text",
        },
        edgeCrossing: {
        ...
        for (const tab of tabs) {
          if (tab.status !== Tab.status.DONE) {
            ...
            if (tab.status === Tab.status.DIRTY) {
              alert(`Parameter changed without a re-run in ${tab.title}`);
            } else {
  ...
  function getWeights() {
    return {
      ...
      edgeLength: parseFloat(PAGE.querySelector("#edge-length-weight").value),
    ...
    switch (target.id) {
      case "generate":
        ...
        if (edgeNumMin < nodeNumMin - 1 || !edgeNumMin) {
          edgeError.innerHTML = `Can't have less than ${nodeNumMin - 1} edges `;
          edgeError.style.display = "block";
          break;
        }
    ...
    for (let tab of tabs) {
      for (let key of Object.keys(tab.files)) {
        ...
        let row = `${tab.title},${key},${executionTime},${evaluatedSolutions},${
          tab.layout
        },${graph.nodes().length},${graph.edges().length},${graph.density()},${
          metrics.nodeOcclusion
        },${metrics.nodeEdgeOcclusion},${metrics.edgeLength},${
          metrics.edgeCrossing
        },${metrics.angularResolution},${w.nodeOcclusion},${
          w.nodeEdgeOcclusion
        },${w.edgeLength},${w.edgeCrossing},${w.angularResolution},${
